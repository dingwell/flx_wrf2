...
* ps                 surface pressure                                 *
*                                                                     *
* aprec              Total accumulated precipitation                  *
*                                                                     *
***********************************************************************
*
...
c local variables
     ...
     &  map_proj_id_dum
      ...
      real dumaprec(0:nxmax-1,0:nymax-1,1,2)
...
c
     ...
     &  map_truelat1_dum, map_truelat2_dum )
      if (ierr .ne. 0) then
          write(*,9100) 'error getting gridinfo for met file', fnamenc
          stop
      end if
...
c     particle position, we will get fractional values anyway.
      ...
      ! 3D-field:
      call read_ncwrfout_1realfield( ierr, idiagaa, fnamenc,
     &  varname, cldfrah(0,0,kbgn,n),
     &  itime,
     &  ndims, ndims_exp, ndims_max,
     &  lendim, lendim_exp, lendim_max )
      if (ierr .ne. 0) then
          write(*,9100) 'error doing ncread of CLDFRA', fnamenc
          stop
      end if
      ! 2D-field:
      do k = kbgn, nuvz
      ...
      do i = 0, nxmin1
        ...
        if( k.eq.kbgn ) then  ! At first level?
          tcc(i,j,1,n) = cldfrah(i,j,k,n)
        else if( tcc(i,j,1,n) .lt. cldfrah(i,j,k,n) ) then ! Subsequent
          tcc(i,j,1,n)=cldfrah(i,j,k,n)
        endif
      end do
      end do
...
c     accumulated precipitation since grid start.
      ...
      varname='RAINC'
      call read_ncwrfout_1realfield( ierr, idiagaa, fnamenc,
     &varname, dumaprec(0,0,1,n),
     &itime,
     &ndims, ndims_exp, ndims_max, 
     &lendim, lendim_exp, lendim_max )
      do j = 0, nymin1
      do i = 0, nxmin1
        aconvprec(i,j,1,n) = aconvprec(i,j,1,n)+dumaprec(i,j,1,n)
      end do
      end do
     ...
     &lendim, lendim_exp, lendim_max )
      do j = 0, nymin1
      do i = 0, nxmin1
        alsprec(i,j,1,n) = alsprec(i,j,1,n)+dumaprec(i,j,1,n)
      end do
      end do
